...
class UserDatabaseHelper {
  ...
  Future<void> deleteCurrentUserData() async {
    ...
    for (final collection in [cartCollectionRef, addressCollectionRef, ordersCollectionRef]) {
  ...
  Future<bool> switchProductFavouriteStatus(String productId, bool newState) async {
    ...
    final docRef = firestore.collection(USERS_COLLECTION_NAME).doc(uid);
    if (newState) {
      await docRef.update({FAV_PRODUCTS_KEY: FieldValue.arrayUnion([productId])});
    } else {
      await docRef.update({FAV_PRODUCTS_KEY: FieldValue.arrayRemove([productId])});
    }
    return true;
  }
  ...
  Future<bool> addAddressForCurrentUser(Address address) async {
    String uid = AuthentificationService().currentUser.uid;
    final ref = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ADDRESSES_COLLECTION_NAME);
  ...
  Future<bool> deleteAddressForCurrentUser(String id) async {
    String uid = AuthentificationService().currentUser.uid;
    final ref = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ADDRESSES_COLLECTION_NAME).doc(id);
    await ref.delete();
    return true;
  }
  ...
  Future<bool> updateAddressForCurrentUser(Address address) async {
    String uid = AuthentificationService().currentUser.uid;
    final ref = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ADDRESSES_COLLECTION_NAME).doc(address.id);
  ...
  Future<CartItem> getCartItemFromId(String id) async {
    String uid = AuthentificationService().currentUser.uid;
    final doc = await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME).doc(id).get();
    final data = doc.data();
  ...
  Future<bool> addProductToCart(String productId) async {
    String uid = AuthentificationService().currentUser.uid;
    final docRef = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME).doc(productId);
  ...
  Future<List<String>> emptyCart() async {
    String uid = AuthentificationService().currentUser.uid;
    final snapshot = await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME).get();
  ...
  Future<bool> removeProductFromCart(String cartItemID) async {
    String uid = AuthentificationService().currentUser.uid;
    final ref = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME).doc(cartItemID);
    await ref.delete();
    return true;
  }
  ...
  Future<List<String>> get orderedProductsList async {
    String uid = AuthentificationService().currentUser.uid;
    final snapshot = await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ORDERED_PRODUCTS_COLLECTION_NAME).get();
    return snapshot.docs.map((doc) => doc.id).toList();
  }

  Future<bool> addToMyOrders(List<OrderedProduct> orders) async {
    String uid = AuthentificationService().currentUser.uid;
    final ref = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ORDERED_PRODUCTS_COLLECTION_NAME);
  ...
  Future<bool> updatePhoneForCurrentUser(String phone) async {
    String uid = AuthentificationService().currentUser.uid;
    await firestore.collection(USERS_COLLECTION_NAME).doc(uid).update({PHONE_KEY: phone});
    return true;
  }
  ...
  Future<bool> uploadDisplayPictureForCurrentUser(String url) async {
    String uid = AuthentificationService().currentUser.uid;
    await firestore.collection(USERS_COLLECTION_NAME).doc(uid).update({DP_KEY: url});
    return true;
  }

  Future<bool> removeDisplayPictureForCurrentUser() async {
    String uid = AuthentificationService().currentUser.uid;
    await firestore.collection(USERS_COLLECTION_NAME).doc(uid).update({DP_KEY: FieldValue.delete()});
    return true;
  }

  Future<String?> get displayPictureForCurrentUser async {
    String uid = AuthentificationService().currentUser.uid;
    final doc = await firestore.collection(USERS_COLLECTION_NAME).doc(uid).get();
